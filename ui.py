...
# Clean up old tokens (prevents reusing the same session)
...
if st.session_state.show_auth and not st.session_state.authenticated:
    st.subheader("1️⃣ Click the button below to log in with Spotify:")
    
    auth_url = sp_oauth.get_authorize_url()
    
    st.markdown(f'<a href="{auth_url}" target="_self"><button style="background-color:#1DB954;color:white;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;font-size:16px;">Login to Spotify</button></a>', unsafe_allow_html=True)
